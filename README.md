...
Este repositorio contiene una aplicación web ligera que ayuda a agrupar archivos con nombres similares en carpetas mediante un asistente de IA que propone un plan de organización y genera un script (Bash o PowerShell) para mover los archivos localmente.

Nota: se han eliminado las referencias comerciales y de marca en la interfaz y en este README. La app sigue usando un servicio de modelo de lenguaje para generar el plan (configurable mediante una API key).

## Qué hace este proyecto

- Escanea los nombres de los archivos en una carpeta seleccionada desde el navegador.
- Pide al servicio de IA que proponga agrupaciones por prefijos significativos (por ejemplo, series, proyectos, etc.).
...
Si quieres que cambie el autor o ajuste el contenido del README con más detalles (ejemplos, capturas, flujos de CI/CD), dime qué prefieres y lo actualizo.
